...
| Q5 | 変更後の仕様に対応するコードは既に存在しますか？（はい / いいえ） | 乖離リスクの判断 |
...
対象の exec-plan ファイルの `## Decision Log` セクションに以下の形式で追記する：
...
- [ ] exec-plan の Decision Log に変更理由が記録されている
- [ ] Q5 が「はい」の場合、`check-doc-freshness` を実行済みである
