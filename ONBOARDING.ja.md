...
- `docs/02_design/` — アーキテクチャ・データモデル
- `docs/03_implementation/` — 不変条件・パターン・依存関係
- `docs/04_quality/` — テスト戦略・レビューチェックリスト
...
> **最低限必要なファイル**: `docs/03_implementation/invariants.md` と `docs/06_ai_context/CONTEXT.md` の 2 つ。この 2 ファイルがないと `/start-feature` が動作しません。
...
| ファイル | 変更箇所 | 具体例 |
|---------|---------|-------|
| `.claude/skills/*/SKILL.md` | ドキュメントパスの参照箇所 | `docs/06_ai_context/CONTEXT.md` → `my-docs/context.md` |
...
> 人が起動するのは上段（**6 ＋ 周期 2**）。`start-feature` は機能ごとに一度、自走を始める前の準備として起動する。下段の検証スキルは `start-feature` / `run-exec-plan` / `pre-pr` / `complete-exec-plan` / `gc` が内部で呼び出す（`update-context` は `complete-exec-plan` / `gc` から）。
...
- 既存プロジェクトなら `docs/03_implementation/invariants.md` を手動作成する
