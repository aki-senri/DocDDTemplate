...
```
MainViewModel
├── ObservableCollection<FileEntryViewModel> Files  ← 再帰走査結果（フラット）
├── ObservableCollection<TagViewModel> Tags
├── ObservableCollection<TagViewModel> ActiveFilters
├── FolderNode? FolderTree                          ← ツリー表示・移動先選択用
├── FolderNode? SelectedMoveDestination
├── ICommand OpenFolderCommand
├── ICommand SelectMoveDestinationCommand
...
{
    /// <summary>folderPath 配下を再帰的に走査して全ファイルを返す。</summary>
    IAsyncEnumerable<FileEntry> GetFilesAsync(string folderPath,
        CancellationToken ct = default);

    /// <summary>folderPath 配下のフォルダツリーを返す（移動先選択用）。</summary>
    IReadOnlyList<FolderNode> GetFolderTree(string folderPath);

    /// <summary>filePaths のファイルを destinationFolder へ移動する。</summary>
