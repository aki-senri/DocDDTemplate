...
- PR #32 作成
- PR #32 のレビュー対応（1回目: Copilot 1 件 ＋ 同種の未指摘 1 件）
- PR #32 のレビュー対応（2回目: 部分 `n/a` 経路。指摘1件から単一ソースの手順まで遡って修正）
...
- **PR #32 2回目のレビュー対応（部分 `n/a` 経路）**: Copilot が「Step 0c は spec の `E2E-NNN` が
  必ず在る前提で書かれており、`/create-spec` を飛ばした小さな変更で非実行可能になる」を指摘
  （suppressed 2件を含め計3件、いずれも同一根因）。**妥当**。AC-012 の免除周回（lap 4）で
  辿ったのは *全列 `n/a`* の documentation-only だけで、**片方だけ `n/a`**（US はあるが
  spec が無い＝`create-exec-plan` が明示的に許している skip パス）を辿っていなかった。
  この経路では「起点なし」ではなく「起点が1層上にある」が正しい。修正:
  - `run-exec-plan` Step 0c — 開く対象を表にした（spec `E2E-NNN` / US ゴール像・主要ユーザー
    ジャーニー / 何も無い）。矛盾行の文言も「scenario or journey」に一般化。
  - `run-exec-plan` Step 1b — `n/a` 行を「片方だけ」「両方」に分割。片方は免除ではない。
  - `run-exec-plan` Step 3a ＋ `ac-sources.md` の再アンカー手順 — **単一ソース側が
    「spec 該当節を開く」と書いていた**のが最深部。手順1を選択表（最も具体的な起点を採る）に
    差し替え、判定表の「起点なし」を「両列とも `n/a`、または表が無い」に限定。
  - `red-first.md` 手順表 step 1 — 「US bullets と spec 該当節」→「その行が指すもの
    （どちらかが `n/a` でありうる）」。
  - `start-feature` Step 1b — R2 のフォールバック（`n/a`／表なしは AC 行のみ）を明記。
    `run-exec-plan` Step 0b には書いたのに同じ pass で書き漏らしていた。
  指摘は1件だが、根因を辿って**単一ソースの手順そのもの**まで直した。3件を個別に潰すと
  `ac-sources.md` が「spec 該当節を開く」と述べたまま残り、次に参照する地点で再発する。
