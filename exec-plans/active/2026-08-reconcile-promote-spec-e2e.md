...
- PR #31 のレビュー対応（Copilot 4 件のうち 3 件が本プラン分）
...
- **ファイル名の経緯（現行名 `2026-08-reconcile-promote-spec-e2e.md`）**。起票時点の CLAUDE.md は
  in-version の reconcile を `YYYY-MM-reconcile.md` としか定めておらず、同名のプランが同月内に既に
  完了・`exec-plans/completed/` へ移動済みだったため、同名で起こすと完了時に上書き衝突する状態だった。
  当座は**旧名 `2026-08-reconcile-2.md`**（連番）で回避し、「同月2本目以降の命名が規約に無い」ことを
  人の判断事項として提示した。
  その後ユーザーが「連番方式を正式とする。ただしできる限り内容が明確なファイル名にする」と決定し、
  `2026-08-convention-gaps.md` の AC-001 で CLAUDE.md に命名規約
  （in-version の当月2本目以降は `YYYY-MM-reconcile-<topic>.md`、連番は最後の手段。
  昇格起点 `-<label>` との判別基準つき）を追加、AC-002 で本ファイルを現行名にリネームした。
  **現在この論点は未解決ではなく、規約として決着している。**

- **PR #31 レビュー対応（Copilot、本プラン分 3 件）**。いずれも妥当と判断し反映した。

  1. **reconcile テンプレートのプレースホルダが `AC-00X` / `E2E-00Y` だった**。リポジトリの
     プレースホルダ表記は `AC-NNN` / `E2E-NNN` で統一されており、`00X` は3桁数字でないため
     **コピペし残すと `spec-gate.py` の `AC-(\d{3}):` にも `check-doc-invariants` Step 5 にも
     引っかからない**（＝その AC がゲートから見えなくなる）。同じテンプレートに「ID を捏造しない」と
     書いておきながら、捏造しても検出されない形を提示していたことになる。`AC-NNN` / `E2E-NNN` に統一。
  2. 報告書式にも同じ `AC-00X` が残っていたため揃えた（指摘は2件だが原因は1つ）。
  3. **本 Decision Log の「ファイル名を `2026-08-reconcile-2.md` にした理由」が現行ファイル名と
     食い違い、命名規約が未整備であるかのように読めた**。命名規約はその後
     `2026-08-convention-gaps.md` の AC-001 / AC-002 で決着済み。旧名であることを明示し、
     決着済みである旨まで含めて書き直した。過去の記録を消さずに現状へ追従させる形にしている
     （Decision Log は履歴であり、後から読む人が現状を誤解しないことが要件のため）。
