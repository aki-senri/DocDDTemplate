...
- PR #31 のレビュー対応（Copilot 4 件のうち 1 件が本プラン分）
...
- **矛盾チェック第2巡（テンプレート全体の自己照合、2026-08-02）**。AC-010 の照合は「単一ソースと
  各地点の**記述**が一致するか」を見ており、**ループを実際に1周させたときの状態遷移**を追っていなかった。
  レビュー用スキルが無いため、`run-exec-plan` を1周（起票→0b→0c→AC実装→E2E AC→pre-pr、および
  中断後の再開）分だけ手で辿り直したところ、記述レベルでは整合しているのに**動かすと破綻する**矛盾が
  6件見つかった。**AC-010 の「全項目 PASS」は、この観点を欠いた検査に対する PASS だった**（判定の
  取り消しではなく、検査範囲の不足）。6件とも修正済み。

  | # | 矛盾 | なぜ破綻するか | 修正 |
  |---|------|---------------|------|
  | 1 | `run-tests` が Step 0c の赤い E2E テストを**通常の失敗**として扱う | ループ中の各 AC 検証で毎回 `run-tests` が呼ばれ、そのたびに仕様照合ゲート（A/B の判断要求）へ落ちる。`run-exec-plan` 側だけ「緑扱い」と書いても、実際に判定するのは `run-tests` なので破綻する | `run-tests` に **Step 2c（expected red）** を新設。「Decision Log に `AC-NNN red-first:` があり、その AC が未チェック」の失敗を失敗と分けて報告し、仕様照合ゲートに渡さない |
  | 2 | 中断後の再開でベースラインが赤になる | 凍結した E2E テストは赤のまま残るため、`run-exec-plan` Step 0 と `start-feature` Step 0 の「赤いベースラインなら開始しない」に必ず引っかかる。**「会話履歴なしでファイルだけから再開できる」規約（CLAUDE.md）と正面から矛盾する** | 両 Step 0 に「expected red は赤いベースラインに数えない（それ以外は全部緑であること）」を追加 |
  | 3 | Step 3 の `[E2E]` カバレッジが、ループ中の未実装 E2E を ❌ hold と判定する | 未実装なのだから赤いのが正しいのに、毎周 hold になる | `run-tests` Step 3 の表に「未チェックの `[E2E]` AC の expected red は ⏳（ループ中は hold にしない／`pre-pr`・`complete-exec-plan` では従来どおり hold）」を追加 |
  | 4 | `[E2E]` AC に到達して**緑になった瞬間**に HALT する | Step 2a の「初回から緑＝AC が既に満たされているかテストが AC を検査していない、判別できなければ HALT (a)」が、成功そのものである状態に適用されてしまう | Step 2a に `[E2E]` AC 専用の表を追加（緑＝機能 AC が積み上がった＝成功。「初回から緑」の分岐は適用しない） |
  | 5 | Step 2a の「初回から緑」で**チェックボックスを直接付ける**手順になっていた | Step 3（check-invariants など）を飛ばして完了扱いになり、ループの検証規約を自分で破る | 「Step 3 を通し、Step 3b でチェックする」に修正 |
  | 6 | Step 2b「Step 2a のテストに触るな」と `red-first.md`「機構的なリファクタは可」が矛盾 | 単一ソースより地点が厳しい＝ドリフトの芽 | 「**期待値**を変えるな（given/when/then が同一の機構的変更は可）」に修正 |

  あわせて `red-first.md` に「実装待ちの赤（expected red）」節を新設し、地点ごとの扱いを単一ソース側に
  集約した（1〜3 の修正が各地点に散らばると、同じ種類のドリフトを次に生む）。
...
- **`post-tool-notify.py` のテストファイル通知を INV-T02 に追従**。フック文言が「テスト**修正**」の
  ...
  スコープを広げない不備修正として CLAUDE.md の規約どおり main 側で対応）。
- **PR #31 レビュー対応（Copilot、本プラン分 1 件）**。上記フック文言の追記部分が
  「既存テストの変更理由も同じ判断ログに記録します」と**説明調**になっており、前後の
  「〜してください」（依頼）と文体が不一致だった。フックの出力は読み手への指示なので、
  依頼形に統一した。
- **検査範囲についての学び**。AC-010 のような「地点間の記述が一致するか」の照合は必要だが十分ではない。
  規約が状態機械（ループ・再開・ゲート）に触れる場合は、**その状態機械を1周ぶん手で辿る**照合を
  併せて行わないと、記述は揃っているのに動かない、という形の矛盾が残る。今回の6件はすべて
  「1周辿る」でしか出てこないものだった。
